...
8. push / PR 作成に進む。作業ブランチの push はユーザーが実行し、PR 作成はユーザーの明示的な承認を得たうえでエージェントが実行してよい。エージェントはいずれについても実行するコマンドと内容を提示し、push はユーザーの実行を待ち、PR 作成は承認を得るまで実行しない(`.claude/rules/pr.md`)
...
確定文言の検証・適用にファイルへの書き込みを伴うコマンドを使う場合、`cp` の対話的な上書き確認(`-i` オプションやそれをデフォルトにするエイリアス)や zsh の `noclobber` は、単体のコマンドとして実行すればエラーメッセージと非ゼロの終了コードで書き込みを拒否するが、複数コマンドをまとめて実行して個々の終了コードを確認していない場合や、標準入力が対話端末に接続されたまま応答を待ち続ける環境では、書き込みが行われなかったことに気づけない、またはコマンドが無反応になることがある。上書きを明示する形で実行する場合、`cp` の `-f` は `-i` を打ち消さない実装がある(GNU coreutils)ため、`-f` を付け足すのではなくエイリアスを経由しない形(`\cp`、`command cp` 等)で実行し、`noclobber` は無効化した `>` 等で書き込む。比較対象のファイルを組み立てる工程についても、生成直後にファイルが空でないこと・意図した内容になっていることを確認してから比較する。
...
- `## 確認済みのこと`: 実行したコマンド(`pnpm ws-check-code` 等)とその結果
    ...
    - タスクの入力が issue でない場合は照合すべき実値が存在しないため、実値単体の検索は実施対象から外し、入力元と実値が存在しないことを記録する。実施していない検査をコマンド例として書かない
