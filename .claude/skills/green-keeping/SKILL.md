...
5. **PR 作成**: PR 作成は手順3 の承認を得たうえでエージェントが実行する。`gh pr create --base develop --head <branch> --title "green keeping<YYYYMMDD>" --label "01.greenkeeping" --body <body>` を実行する。ユーザーの push が済んでいない場合は実行せず、push を待つ
6. **完了報告**: 更新したカテゴリ・主要パッケージ・発生した修正内容を報告する。作成した PR の URL もあわせて報告する
...
- 「develop への PR 作成」の手順3（PR 提示と承認）は省略しない。push は origin への共有操作、PR 作成は GitHub 上に公開される操作であり、いずれも取り消しにくい／他者に見える変更のため。push の実行はユーザーが行い（`.claude/settings.json` の deny に `Bash(git push:*)` がある）、PR 作成は承認を得たうえでエージェントが実行する（`.claude/rules/pr.md`。`.claude/settings.json` の ask に `Bash(gh pr create:*)` がある）
