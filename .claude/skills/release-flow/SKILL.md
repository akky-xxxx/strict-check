---
name: release-flow
description: 「1. バージョニングをインクリメント（作業ブランチ上）」「2. 作業ブランチ→develop PR作成」「3. develop→main release PR作成」「4. タグ付け」の4段階でリリース手順を確実に実行するためのスキル。対象パッケージ抽出・次バージョン提案の根拠付け・release PR（`release@<YYYYMMDD>`）のタイトル/ラベル/本文フォーマット・`pnpm tagging`（origin への破壊的操作 = npm publish トリガー）を含む。バージョンの確定・作業ブランチの push・全PRのマージは必ずユーザー自身が行い、エージェントは代行しない。PR 作成とタグ付けはユーザーの明示的な承認を得たうえでエージェントが実行する。各ステップの確認を省略しない。`/release-flow` で明示的に起動する他、パッケージのリリース・バージョンアップ作業を検知した際にも使う。
---
...
1. **PR 作成**: CLAUDE.md の PR テンプレート（日本語6セクション: 変更概要/背景・目的/変更内容/影響範囲/動作確認/補足）に従い、作業ブランチ→develop の通常 PR を組み立てる
    ...
    - push はユーザーが実行する。`git push -u origin <作業ブランチ>` を提示し、実行はユーザーに委ねる。PR 作成はタイトル/base/head/本文と実行するコマンド（`gh pr create --base develop --head <作業ブランチ> --title ... --body ...`）を提示し、ユーザーの明示的な承認を得たうえでエージェントが実行する。push が済んでいない場合は PR 作成に進まない
...
1. **release PR 作成**: 上記「release PR の規約」に沿って以下を組み立てる
    ...
    - PR 作成はタイトル/ラベル/base/head/本文と実行するコマンド（`develop` は既に origin に反映済みのため push は不要。`gh pr create --base main --head develop --title ... --label ... --body ...`）を提示し、ユーザーの明示的な承認を得たうえでエージェントが実行する
...
2. **タグ付け（破壊的操作・確認必須）**: main ブランチ上で対象パッケージごとに `pnpm --filter <package-name> run tagging` を実行する。実行前に対象パッケージ名・バージョン・作成されるタグ文字列（`<package-name>/v<version>`）を明示し、ユーザーの明示的な承認を得る。承認前に実行しない。承認を得たらエージェントが実行する
...
- **作業ブランチの push はユーザーが実行する。** エージェントは実行するコマンドを提示するところまでを担当する（`.claude/settings.json` の deny に `Bash(git push:*)` がある）。ステージ4のタグ push は `scripts/tagging/index.mjs` が内部で行う別の操作であり、承認を得たうえでエージェントが実行する
...
- ステージ2・ステージ3の PR 作成は GitHub 上に公開される操作であり、承認を得たうえでエージェントが実行してよいが、提示と承認を省略しない
- ステージ4のタグ付け（`pnpm tagging` の実行）は origin への push かつ npm publish のトリガーであり、本リポジトリで最も取り消しが難しい操作の一つ。承認を得たうえでエージェントが実行してよいが、実行前確認を絶対に省略しない
